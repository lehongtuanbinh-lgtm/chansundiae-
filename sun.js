...
// ĐÃ THAY ĐỔI /sun ĐỂ HIỂN THỊ ĐỊNH DẠNG MỚI MÀ KHÔNG LÀM MẤT LOGIC CŨ
app.get('/sun', async (req, res) => {
  try {
    ...
    const prevRound = sunData[0]; // Lấy thông tin phiên trước từ data API trả về
    ...
    // Quy đổi độ tin cậy % thành chữ như yêu cầu
    let do_tin_cay_text = "Bình thường";
    if (result.confidence >= 80) do_tin_cay_text = "Rất mạnh";
    else if (result.confidence >= 70) do_tin_cay_text = "Mạnh";
    else if (result.confidence >= 60) do_tin_cay_text = "Khá";
    else do_tin_cay_text = "Thấp";
    
    // Trả về JSON theo đúng định dạng mẫu
    res.json({
      phien: prevRound.Phien,
      xuc_xac_1: prevRound.xuc_xac_1,
      xuc_xac_2: prevRound.xuc_xac_2,
      xuc_xac_3: prevRound.xuc_xac_3,
      tong: prevRound.tong,
      ket_qua: prevRound.Ket_qua,
      phien_hien_tai: nextPhien,
      du_doan: result.prediction, // Giữ nguyên 'Tài' hoặc 'Xỉu' có viết hoa
